...
function woo_add_paypal_fee($cart) {
     ...
     // DEBUG LOGGING
     write_log('=== GEBÜHREN DEBUG ===');
     write_log('User: ' . $user->user_login);
     write_log('Rollen: ' . implode(', ', (array) $user->roles));
     write_log('ist_gutschein_shop: ' . ($is_gutschein_shop ? 'JA' : 'NEIN'));
     write_log('Gebühr %: ' . $percent);
     write_log('Gebühr €: ' . $fee);
     write_log('post_data[billing_options]: ' . (isset($post_data['billing_options']) ? $post_data['billing_options'] : 'NICHT DA'));
     write_log('$_POST[billing_options]: ' . (isset($_POST['billing_options']) ? $_POST['billing_options'] : 'NICHT DA'));
               ...
               if((array_key_exists('billing_options', $post_data) && !empty($post_data['billing_options']) && $post_data['billing_options'] == 'firmenkauf' ) || (array_key_exists('billing_options', $_POST) && !empty($_POST['billing_options']) && $_POST['billing_options'] == 'firmenkauf')) {
                    write_log('Firmenkauf erkannt');
                    // Keine Bearbeitungsgebühr für gutschein-shop Rolle bei Firmenkauf
                    if ( !$is_gutschein_shop ) {
                         write_log('GEBÜHR HINZUGEFÜGT');
                         $cart->add_fee( 'Bearbeitungsgebühr', $fee, true, 'Steuern Fee' );
                    } else {
                         write_log('GEBÜHR NICHT HINZUGEFÜGT (gutschein-shop)');
                    }
               } else {
                    write_log('NICHT Firmenkauf oder billing_options nicht gesetzt');
               }
}
