...
// -----------------------------------------------------------------------------
	...
	protected function decodeStream($sdic, $stream) {
		...
		foreach ($filters as $filter) {
			if (in_array($filter, TM_TCPDF_FILTERS::getAvailableFilters())) {
				try {
					$stream = TM_TCPDF_FILTERS::decodeFilter($filter, $stream);
				} catch (Exception $e) {
					$emsg = $e->getMessage();
	...
	public function Error($msg) {
		if ($this->cfg['die_for_errors']) {
			die('<strong>TM_TCPDF_PARSER ERROR: </strong>'.$msg);
		} else {
			throw new Exception('TM_TCPDF_PARSER ERROR: '.$msg);
		}
	}

} // END OF TM_TCPDF_PARSER CLASS
