...
if (!defined('TM_K_PATH_MAIN')) {
	if (!defined('TM_K_PATH_MAIN')) define ('TM_K_PATH_MAIN', dirname(__FILE__).'/');
}

if (!defined('TM_K_PATH_FONTS')) {
	if (!defined('TM_K_PATH_FONTS')) define ('TM_K_PATH_FONTS', TM_K_PATH_MAIN.'fonts/');
}

if (!defined('TM_K_PATH_URL')) {
	$k_path_url = TM_K_PATH_MAIN; // default value for console mode
	if (isset($_SERVER['HTTP_HOST']) AND (!empty($_SERVER['HTTP_HOST']))) {
		...
		$k_path_url .= str_replace( '\\', '/', substr(TM_K_PATH_MAIN, (strlen($_SERVER['DOCUMENT_ROOT']) - 1)));
	}
	if (!defined('TM_K_PATH_URL')) define ('TM_K_PATH_URL', $k_path_url);
}

if (!defined('TM_K_PATH_IMAGES')) {
	$tcpdf_images_dirs = array(TM_K_PATH_MAIN.'examples/images/', TM_K_PATH_MAIN.'images/', '/usr/share/doc/php-tcpdf/examples/images/', '/usr/share/doc/tcpdf/examples/images/', '/usr/share/doc/php/tcpdf/examples/images/', '/var/www/tcpdf/images/', '/var/www/html/tcpdf/images/', '/usr/local/apache2/htdocs/tcpdf/images/', TM_K_PATH_MAIN);
	foreach ($tcpdf_images_dirs as $tcpdf_images_path) {
		if (@file_exists($tcpdf_images_path)) {
			if (!defined('TM_K_PATH_IMAGES')) define ('TM_K_PATH_IMAGES', $tcpdf_images_path);
			break;
		}
	}
}
...
if (!defined('PDF_HEADER_LOGO')) {
	$tcpdf_header_logo = '';
	if (@file_exists(TM_K_PATH_IMAGES.'tcpdf_logo.jpg')) {
		$tcpdf_header_logo = 'tcpdf_logo.jpg';
	}
	if (!defined('PDF_HEADER_LOGO')) define ('PDF_HEADER_LOGO', $tcpdf_header_logo);
}
...
if (!defined('PDF_HEADER_LOGO_WIDTH')) {
	if (!empty($tcpdf_header_logo)) {
		...
		if (!defined('PDF_HEADER_LOGO_WIDTH')) define ('PDF_HEADER_LOGO_WIDTH', 0);
	}
}

if (!defined('TM_K_PATH_CACHE')) {
	$TM_K_PATH_CACHE = ini_get('upload_tmp_dir') ? ini_get('upload_tmp_dir') : sys_get_temp_dir();
	if (substr($TM_K_PATH_CACHE, -1) != '/') {
		$TM_K_PATH_CACHE .= '/';
	}
	if (!defined('TM_K_PATH_CACHE')) define ('TM_K_PATH_CACHE', $TM_K_PATH_CACHE);
}
...
if (!defined('K_TIMEZONE')) {
	if (!defined('K_TIMEZONE')) define('K_TIMEZONE', @date_default_timezone_get());
}
