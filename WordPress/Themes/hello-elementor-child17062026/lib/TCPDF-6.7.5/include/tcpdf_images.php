...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 ...
	 * This function requires GD library and write access to the directory defined on TM_K_PATH_CACHE constant.
	...
	public static function _parsejpeg($file) {
		...
		if (!@TM_TCPDF_STATIC::file_exists($file)) {
			return false;
		}
		$a = getimagesize($file);
		if (empty($a)) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (TM_TCPDF_STATIC::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = TM_TCPDF_STATIC::_freadint($f);
		$h = TM_TCPDF_STATIC::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$data = '';
		$icc = false;
		$n = TM_TCPDF_STATIC::_freadint($f);
		do {
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = TM_TCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = TM_TCPDF_STATIC::rfread($f, $n);
				if ($ct == 0) { // DeviceGray
					$trns = array(ord($t[1]));
				...
				$data .= TM_TCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'iCCP') {
				// skip profile name
				$len = 0;
				...
				$icc = TM_TCPDF_STATIC::rfread($f, ($n - $len - 2));
				// decompress profile
