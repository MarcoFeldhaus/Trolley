...
class TM_TCPDF_STATIC {
	...
	public static function getObjFilename($type='tmp', $file_id='') {
		return tempnam(TM_K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.md5(TM_TCPDF_STATIC::getRandomSeed()).'_');
	}
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		if (isset($prop['fillColor'])) {
			if (is_array($prop['fillColor'])) {
				...
				$opt['mk']['bg'] = TM_TCPDF_COLORS::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
			}
		}
		...
		if (isset($prop['strokeColor'])) {
			if (is_array($prop['strokeColor'])) {
				...
				$opt['mk']['bc'] = TM_TCPDF_COLORS::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
			}
		}
	...
	public static function fixHTMLCode($html, $default_css, $tagvs, $tidy_options, &$tagvspaces) {
		...
		if (TM_TCPDF_STATIC::empty_string($tidy_options)) {
			$tidy_options = array (
				'clean' => 1,
				'drop-empty-paras' => 0,
		...
		if (!TM_TCPDF_STATIC::empty_string($tagvs)) {
	...
	public static function getHyphenPatternsFromTEX($file) {
		...
		foreach($patterns_array as $val) {
			if (!TM_TCPDF_STATIC::empty_string($val)) {
				$val = trim($val);
				$val = str_replace('\'', '\\\'', $val);
